...
enum ps4_packet_index {
  ...
  packet_index_sensor_temperature = 24,
...
enum ps4_touch_packet_index {
  packet_index_touch_count = 45,
  packet_index_touch_timestamp = 46,
  packet_index_touch_0 = 47,
  packet_index_touch_1 = 51
};
...
ps4_touchpad_t parsePacketTouchpad(uint8_t* packet, size_t packetLength);
...
static uint8_t latestPacketBuffers[2][PS4_HID_INPUT_PACKET_SIZE];
static size_t latestPacketLengths[2];
static volatile uint8_t latestPacketBufferIndex = 0;
static volatile bool latestPacketAvailable = false;
...
void parsePacket(uint8_t* packet, size_t packetLength) {
  ps4_t prev_ps4 = ps4;
  uint8_t nextBufferIndex = latestPacketBufferIndex ^ 1U;

  /*
   * Copy before parsing; the L2CAP callback frees packet immediately after
   * this function returns.  Keep parsePacket's existing parsing and event
   * behavior unchanged, including its handling of short initial packets.
   */
  if (packet != NULL && packetLength <= PS4_HID_INPUT_PACKET_SIZE) {
    memcpy(latestPacketBuffers[nextBufferIndex], packet, packetLength);
    latestPacketLengths[nextBufferIndex] = packetLength;
    latestPacketBufferIndex = nextBufferIndex;
    latestPacketAvailable = true;
    ps4.latestPacket = latestPacketBuffers[nextBufferIndex];
    ps4.latestPacketLength = packetLength;
  } else {
    latestPacketAvailable = false;
    ps4.latestPacket = NULL;
    ps4.latestPacketLength = 0;
  }
  ...
  ps4.touchpad = parsePacketTouchpad(packet, packetLength);
  ps4_event_t ps4Event = parseEvent(prev_ps4, ps4);

  ps4PacketEvent(ps4, ps4Event);
}

uint8_t* ps4LatestPacket() {
  return latestPacketAvailable ? latestPacketBuffers[latestPacketBufferIndex] : NULL;
}

size_t ps4LatestPacketLength() {
  return latestPacketAvailable ? latestPacketLengths[latestPacketBufferIndex] : 0;
}
...
/********************/
ps4_touchpad_t parsePacketTouchpad(uint8_t* packet, size_t packetLength) {
  ps4_touchpad_t touchpad = { 0 };
  const uint8_t touchIndexes[2] = {
    packet_index_touch_0,
    packet_index_touch_1
  };

  /* Two contacts require bytes through packet[54]. */
  if (packetLength <= packet_index_touch_1 + 3) {
    return touchpad;
  }

  touchpad.count = packet[packet_index_touch_count];
  touchpad.timestamp = packet[packet_index_touch_timestamp];

  for (uint8_t i = 0; i < 2; ++i) {
    uint8_t index = touchIndexes[i];
    uint8_t idAndInactive = packet[index];
    touchpad.touch[i].id = idAndInactive & 0x7F;
    touchpad.touch[i].active = (idAndInactive & 0x80) == 0;
    touchpad.touch[i].x = packet[index + 1] | ((packet[index + 2] & 0x0F) << 8);
    touchpad.touch[i].y = (packet[index + 2] >> 4) | (packet[index + 3] << 4);
  }
...
ps4_sensor_t parsePacketSensor(uint8_t* packet) {
  ...
  ps4Sensor.temperature = packet[packet_index_sensor_temperature];
